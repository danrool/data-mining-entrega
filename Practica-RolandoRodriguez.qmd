...
```{r}
  ...
  df_madrid_estimado <- df_madrid_clean 
  
  df_madrid_estimado$Square.Meters[is.na(df_madrid_estimado$Square.Meters)] <- metros_habitacion_adicional
...
```{r}
  ...
  head(df_madrid_estimado)

  df_pca <- df_madrid_estimado[, c("Accommodates", "Bathrooms", "Bedrooms", "Beds", "Price", "Guests.Included", "Extra.People", "Review.Scores.Rating", "Latitude", "Longitude", "Square.Meters")]

  df_pca <- na.omit(df_pca)
  
  head(df_pca)
  
  pca_madrid <- prcomp(df_pca, center = TRUE, scale. = TRUE)
...
```{r}
  ...
  plot(pca_madrid $sdev ^2 / sum(pca_madrid $sdev ^2), main="Importancia de componentes")
...
```{r}
    ...
    # Tomo los 4 componentes mas significativos
    
    pca_df = pca_madrid$x[,1:4]
    pca_apartamento = pca_apartamento[,1:4]
  
    
    df_pca[id_test,]


    Apc<-pca_madrid$x[,1:4]
